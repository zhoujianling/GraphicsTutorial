...
void Camera::Pitch(float angle)
{
	...
	const auto direction_look_at = glm::normalize(vec3(look_at_[0] - position_[0], look_at_[1] - position_[1], look_at_[2] - position_[2]));
	const auto right_hand = glm::normalize(glm::cross(direction_look_at, up_));
	Rotate(angle, right_hand);
}
...
void Camera::Rotate(const float angle, const glm::vec3& axis)
{
	...
	look_at_ = vec3(look_at_[0] + direction_look_at[0], look_at_[1] + direction_look_at[1], look_at_[2] + direction_look_at[2]);
	view_matrix_ = glm::lookAt<float, glm::defaultp>(position_, look_at_, up_);
}
